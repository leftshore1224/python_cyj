! PROGRAM BERRY_KIM Version 0.9 (f77) for VASP
...
! last update and bug fixes : 2016. Jun. 27. by H.-J. Kim & Y.-K. Kang et al.,
...
!#undef  MPI_USE
      ...
      integer, allocatable:: i_half_klist(:)
      ...
      ver_tag="# BERRY_KIM (Ver 0.92), by Hyun-Jung Kim. 2015. Jun. 28."
      pi=4.*atan(1.)
      kperiod=2
      nkx=4 ;nky=4 
...
!!$*  reading general informations
     ...
     &   iwf,ikwf,ng,rs,imag)
      ...
      if(myrank == 0)then
#endif
      write(6,'(A,A)')"# File reading... : ",filename
#ifdef MPI_USE
      endif
#endif
      ...
      if(myrank == 0)then
#endif 
      write(6,'(A,I9)')"# TOTAL RECORD LENGTH = ",irecl
#ifdef MPI_USE
      endif
#endif 
      allocate(berrycurv(nk),wklist(3,nk),nplist(nk))
      allocate(berrycurv_tot(nk))
      allocate(xberrycurv(kperiod*2*kperiod*2*nk))
      allocate(w_half_klist(3,nk),i_half_klist(nk))
      do ik=1,nk 
       ne_temp0=ne
       ne=0
       irec=3+(ik-1)*(nband+1)
...
#endif
      allocate(ig(3,npmax),wklp(3,5,nk))
     ...
     &        nplist,ni,nj,irecl,filename,foname)
      ...
      if(iwf .ge. 1)then ! plotting wavefunction
       ilp=1
       ikk(ilp)=ikwf
       iband=iwf
     ...
     &     npl,nmax,irecl,filename,foname,nx,ny,nz,iband,ikk,1,rs,imag)
      ...
      if(iz .eq. 1) then
       ...
       call z2_fukui(a1,a2,a3,b1,b2,b3,
     &       kperiod,nbmax,npmax,ecut,ispinor,ispin,nband,ne,nk,wklist,
     &       nplist,nini,nmax,irecl,filename,foname,iz,nkx,nky,ns,ihf,
     &       dSkxky)
      ...
      if(myrank == 0)then
        ...
        do ik=ink, ifk     !ik loop.
         call klpfind(wkk, isp,ik,nk,nband,nkx,nky,0,0) !k-loop(4pts) of ik-kpoint (K)
         write(6,*)" "
  ...
  490    format("#* Closed loop for KPOINT",I5," : (",3F9.5,"  )")
     ...
     &                  nband,
     &                  iz,w_half_klist,i_half_klist,0,0) !find K-loop for ik
...
!    &          ') = <u_i,K',I1,'(r)|u_j,K',I1,'(r)>')
  ...
  600    format('#===>PI_S[det(S(K_s,K_s+1))] =',F16.8,'  +',F16.8,' i')
         ...
         elseif(iz .eq. 1)then
          berrycurv(ik)=+(    aimag(log(detS(1))) 
     &                  +     aimag(log(detS(2))) 
     &                  +     aimag(log(detS(3))) 
     &                  +     aimag(log(detS(4))))/(2.*pi)
     &                  -    (aimag(log(detLOOP)))/(2.*pi)
          if(wklp(2,1,ik) .gt. 0 .and. wklp(2,1,ik) .lt. 0.5 ) then
           rnfield=rnfield+berrycurv(ik)
          endif
         endif         
     ...
     &                                    berrycurv(ik)
         ...
         write(6,'(A)')"#      kx        ky        kz(A^-1)
     &   n-field strength      "
         write(6,'(A,3F11.6,A,F8.3)')'#',(recivec(i,ik),i=1,3),"     ",
     &                               berrycurv(ik)
!    &                               nint(berrycurv(ik))
         endif
...
!!$*  SUMMARIZTION and output the results..
     ...
     &                  mpi_sum,0,mpi_comm_world,ierr)
        ...
        if(myrank==0)then   !write info only if myrank == 0
        write(6,'(A,F16.6)')"# Chern Number =    ",
     &                      chernnumber_tot
        endif
#else
        ...
        do ib2=-1*(kperiod-1)+1,kperiod 
         do ib1=-1*(kperiod-1)+1,kperiod  ! you may adjust these values as you wish..
          do ik=1,nk
           ...
           xberrycurv(kk)=berrycurv(ik)
           kext=kk
          enddo
         enddo
        enddo
        
!$$*  sorting k-points and the corresponding berrycurvature for the 
!     periodically repeated data grid
       ...
       if(myrank==0)then
        ...
        do k=kext-1,1,-1  ! sorting kx
         do j=1,k
          if(xrecivec(1,j+1) .gt. xrecivec(1,j))then
           ...
           xrecilat(:,j+1)=xb(:)
           xtemp=xberrycurv(j)
           xberrycurv(j)=xberrycurv(j+1)
           xberrycurv(j+1)=xtemp
          endif
         enddo
        enddo
        ...
        open(31,file=fonameo,status='unknown')
        write(31,'(A,A)')   "# File reading...  : ",filename
        write(31,'(A,I9)')"# TOTAL RECORD LENGTH = ",irecl
        if (ispinor .eq. 2)then
         write(31,'(A,I6,A)')"# ISPIN            : ",ispin,
     &                       " (LSORBIT = .TRUE.)"
         else
          write(31,'(A,I6,A)')"# ISPIN            : ",ispin,
     &                        " (LSORBIT = .FALSE.)"
        endif
        write(31,'(A,F11.4)')  "# ENCUT (eV)       : ",ecut
        write(31,'(A,I6)')     "# NKPOINT          : ",nk
        write(31,'(A,I6,A,I4)')"#  K-GRID          : ",nkx,"   X",nky
        write(31,'(A,I6)')     "# NBANDS           : ",nband
        write(31,'(A,3F13.6)') "# RECIVEC B1 (A^-1): ",(b1(i),i=1,3)
        write(31,'(A,3F13.6)') "# RECIVEC B2       : ",(b2(i),i=1,3)
        write(31,'(A,3F13.6)') "# RECIVEC B3       : ",(b3(i),i=1,3)
        write(31,'(A,F13.6)')   "#  dk^2 = |dk1xk2| = ",dSkxky
        write(31,*)" "
        if(nini .eq. nmax) then
         write(31,'(A,I4)')"# Chern Number for the BAND : ",nmax
         else
          write(31,'(A,I4,A,I4)')"# Chern Number for the BANDS : ",nini,
     &                           "    -  ",nmax
        endif
        write(31,'(A)')"# Chern Number is sum of 
     &Berry Curvature over 1BZ"

#ifdef MPI_USE
        write(31,'(A,F16.6)')"# Chern Number =   ",
     &                       chernnumber_tot
#else
        write(31,'(A,F16.6)')"# Chern Number =   ",chernnumber
        write(31,'(A,4F16.6)')"# MAXVAL of BERRYCURV at kx,ky,kz 
     &(in reci)= ",(berrymax(i),i=1,4)
        write(31,'(A,4F16.6)')"# MINVAL of BERRYCURV at kx,ky,kz 
     &(in reci)= ",(berrymin(i),i=1,4)
#endif

        write(31,'(A)')"# Berry Curvature (A^-2) :
     &-Im[logPI_S(det(S(K_s,K_s+1)))]/dk^2"
        if(iz .eq. 1)then
         write(31,'(A,I3)')"# Z2 invariant :",
     &                       mod(abs(nint(rnfield)),2)
        write(31,'(A)')"# (cart) kx        ky        kz(A^-1)
     ...
     &   Berry Curvature (A^-2),   (recip)kx        ky        kz"
        ...
        do ik=1,kext
         if(iz .eq. 0)then
         write(31,'(3F11.6,A,F16.6,A,3F11.6)')(xrecivec(i,ik),i=1,3),
     &        "     ",xberrycurv(ik),"                  ",
     &        (xrecilat(i,ik),i=1,3)
         elseif(iz .eq. 1)then
         write(31,'(3F11.6,A,F8.3,A,3F11.6)')(xrecivec(i,ik),i=1,3),
     &        "     ",xberrycurv(ik),"                  ",
     &        (xrecilat(i,ik),i=1,3)
!    &        "     ",nint(xberrycurv(ik)),"                  ",
         endif
...
!!! ######### LOOP for OPTICAL SELECTIVITY ###########################################
       else if(icd.eq.1)then   ! calculate optical selectivity
        ...
        do ib2=-1*(kperiod-1)+1,kperiod
         do ib1=-1*(kperiod-1)+1,kperiod  ! you may adjust these values as you wish..
          do ik=1,nk
           ...
           xrecilat(2,kk)=recilat(2,ik) +(ib2-1)
           xrecilat(3,kk)=recilat(3,ik)
           xselectivity(kk)=selectivity(ik)
           kext=kk
          enddo
         enddo
        enddo
...
!     periodically repeated data grid
        ...
        do k=kext-1,1,-1  ! sorting kx
         do j=1,k
          if(xrecivec(1,j+1) .gt. xrecivec(1,j))then
           ...
           xrecilat(:,j)=xrecilat(:,j+1)
           xrecilat(:,j+1)=xb(:)
           xtemp=xselectivity(j)
           xselectivity(j)=xselectivity(j+1)
           xselectivity(j+1)=xtemp
          endif
         enddo
        enddo
        do k=kext-1,1,-1  ! sorting ky
     ...
     &                        " (LSORBIT = .FALSE.)"
        ...
        write(31,'(A,I4,A,I4)')"# OPTICAL SELECTIVITY BETWEEN BANDS: ",
     &                         nini,"    -  ",nmax
        write(31,'(A)')"# n(k,w_cv)= |P(k,s,cv,+)|^2 - |P(k,s,cv,-)|^2"
        write(31,'(A)')"#            ---------------------------------"
        write(31,'(A)')"#            |P(k,s,cv,+)|^2 + |P(k,s,cv,-)|^2"
        write(31,'(A)')"#  The TRANSITION MATRIX ELEMENT P ="
        write(31,'(A)')"#   P(k,s,cv,+ or -) = 1/sqrt(2)[p_x(k,cv,s) +
     &(or -) i*p_y(k,cv,s)]"
        write(31,'(A)')"#  THE INTERBAND TRANSITION MATRIX p_x,y ="
        write(31,'(A)')"#   p_x,y(k,cv,s)=<psi(k,c,s)|-i*hbar*1/dx(y)|
     &psi(k,v,s>"
        write(31,'(A,4F16.6)')"# MAXVAL of SELECTIVITY at kx,ky,kz 
     &(in reci)= ",(selectivitymax(i),i=1,4)
        write(31,'(A,4F16.6)')"# MINVAL of SELECTIVITY at kx,ky,kz 
     &(in reci)= ",(selectivitymin(i),i=1,4)
        write(31,'(A)')"# (cart) kx     ky     kz(A^-1)
     &    selectivity(n(k))    (recip)kx      ky      kz"
        do ik=1,kext
         write(31,'(3F11.6,A,F10.6,A,3F11.6)')(xrecivec(i,ik),i=1,3),
     &        "   ",xselectivity(ik),"       ",
     &        (xrecilat(i,ik),i=1,3)
        enddo
        close(31)
...
!!! ######### LOOP END for OPTICAL SELECTIVITY ###########################################
      ...
      if(iskp .eq. 1)call write_special_kpoint(b1,b2,b3)
#ifdef MPI_USE
      if(myrank==0)then
#endif
      write(6,'(A)')"# DONE! "
...
!!$*  subroutine for computing Z2 invariant using Fukui's lattice method
      subroutine z2_fukui(a1,a2,a3,b1,b2,b3,
     &       kperiod,nbmax,npmax,ecut,ispinor,ispin,nband,ne,nk,wklist,
     &       nplist,nini,nmax,irecl,filename,foname,iz,nkx,nky,ns,ihf,
     &       dSkxky)
      ...
      complex*16, allocatable :: Siju(:,:),Sijd(:,:),Sijt(:,:)
      dimension wk(3),nbmax(3),xb(3),wkk(3,5),wklp(3,5,nk*4)
      ...
      complex*16 detS(4),detA,detLOOP
      real*8  wklist(3,nk),wnklist(3,nk*4),w_half_klist(3,nk)
      real*8  recivec(3,nk*4),recilat(3,nk*4)
      real*8  xrecivec(3,kperiod*2*kperiod*2*nk*4)
      real*8  xrecilat(3,kperiod*2*kperiod*2*nk*4)
      real*8  rnfield(nk*4),xrnfield(kperiod*2*kperiod*2*nk*4)
      dimension a1(3),a2(3),a3(3),b1(3),b2(3),b3(3),a2xa3(3),sumkg(3)
...
! set new kpoints : TR partners
     ...
     &                                .and. (wk(2)-.0) .gt. del)then
      ...
      do isp=1,ispin   ! ispin start
       ...
       write(6,*)" "
       rnnfield=0.
       do ik=1,nnk     ! ik    start
        call klpfind(wkk, isp,ik,nk,nband,nkx,nky,wnklist,ihf) !k-loop(4pts) of ik-kpoint (K)
        write(6,*)" "
        write(6,700) ik,(wkk(i,1),i=1,3)
  700   format("#* Closed loop for KPOINT",I5," : (",3F9.5,"  )")
        call kindxfind(ikk,isgg,npl,itr,itrim, wnklist,nplist,wkk,nnk,
     &                nband,
     &                iz,w_half_klist,i_half_klist,i_trim_klist,nhf) !find index

        write(6,701)(ikk(i),i=1,5)
  701   format('# K1>K2>K3>K4>K5=K1 (k-index):',
     &          I5,'  >',I5,'  >',I5,'  >',I5,'  >',I5)
        do j=1,5
         write(6,702)j,(wkk(i,j),i=1,3),j,(isgg(i,j),i=1,2),itr(j),
     &               itrim(j)
  702    format('# K',I1,' = (',3F9.5,'  )',
     &        ', G',I1,' = (n1,n2) = (',I1,',',I1,')',' TR=',I1,
     &        ' TRM=',I1)
        enddo
        wklp(:,:,ik) = wkk(:,:)  ! closed loop (C) for each K : RECIVEC
...
!!$* get overlap matrix S_ij(k,k+1) over the C and PI_s [detS(k_s,k_s+1)], s=1,4
  ...
  704   format('#===>PI_S[det(S(K_s,K_s+1))] =',F16.8,'  +',F16.8,' i')
        rnfield(ik)=+sum(aimag(log(detS(1:4))))/(2.*pi)
     &               - (aimag(log(detLOOP)))/(2.*pi)
        if((wklp(2,1,ik)+wklp(2,3,ik))/2. .gt. -0.00001 .and. 
     &    (wklp(2,1,ik)+wklp(2,3,ik))/2. .lt. 0.500001 ) then
...
!     periodically repeated data grid
     ...
     &   n-field strength      ,   (recip)kx        ky        kz"
       ...
       do ik=1,kext
        write(32,'(3F11.6,A,F8.3,A,3F11.6)')(xrecivec(i,ik),i=1,3),
     &        "     ",xrnfield(ik),"                  ",
     &        (xrecilat(i,ik),i=1,3)
       enddo
...
!!$*  subroutine for getting planewave coefficient for given k & n
     ...
     &                                npmax,itrim,del)
      ...
      if(itrim(iilp) .eq. 0)then
       read(10,rec=(3+(ikk(iilp)-1)*(nband+1)+
     &             nk*(nband+1)*(isp-1)+nni))(coeff(i),i=1,npl(iilp))
     ...
     &             nk*(nband+1)*(isp-1)+nni))(coeff(i),i=1,npl(iilp))
      ...
      elseif(itrim(iilp) .ge. 1 .and. mod(nni,2) .eq. 0)then
       read(10,rec=(3+(ikk(iilp)-1)*(nband+1)+
     &           nk*(nband+1)*(isp-1)+nni-1))(coeff(i),i=1,npl(iilp))
       if(itrim(iilp) .eq. 1)isgg(1,iilp)=1
       if(itrim(iilp) .eq. 2)isgg(2,iilp)=1
       if(itrim(iilp) .eq. 3)isgg(:,iilp)=1
       if(itrim(iilp) .eq. 4)isgg(:,iilp)=0
      endif
...
!!$*  subroutine for getting incnt with given G vector 
     ...
     &        (ig1p-isgg(1,iilp)+nbmax(1))+1
       elseif(itrim(iilp) .ge. 1 .and. mod(ni,2) .eq. 1)then
        incnt=(+ig3p+nbmax(3))*(2*nbmax(2)+1)*(2*nbmax(1)+1)+
     &        (+ig2p-isgg(2,iilp)+nbmax(2))*(2*nbmax(1)+1)+
     &        (+ig1p-isgg(1,iilp)+nbmax(1))+1
       elseif(itrim(iilp) .ge. 1 .and. mod(ni,2) .eq. 0)then
        incnt=(-ig3p+nbmax(3))*(2*nbmax(2)+1)*(2*nbmax(1)+1)+
     &        (-ig2p-isgg(2,iilp)+nbmax(2))*(2*nbmax(1)+1)+
     &        (-ig1p-isgg(1,iilp)+nbmax(1))+1
       endif
      elseif(itr(iilp) .eq. 1)then
       incnt=(-ig3p+nbmax(3))*(2*nbmax(2)+1)*(2*nbmax(1)+1)+
     &       (-ig2p-isgg(2,iilp)+nbmax(2))*(2*nbmax(1)+1)+
     &       (-ig1p-isgg(1,iilp)+nbmax(1))+1
      endif
...
!!$*  subroutine for finding k-loop set for certain "k"-point
      subroutine klpfind(wkk, isp,ik,nk,nband,nkx,nky,wnklist,ihf)
      implicit real*8(a-h,o-z)
      dimension wkk(3,5),wk(3),dk1(3),dk2(3)
      real*8 occ(nband),wnklist(3,nk*4)
      real*16 ener(nband)
      dk1=0.;dk2=0. !;ne=0 
...
!!$*  subroutine for finding k-point index for certain wk
     ...
     &                     i_half_klist,i_trim_klist,nhf)
      implicit real*8(a-h,o-z)
      dimension wkk(3,5),ikk(5),isgg(2,5),wklist(3,nk),wk(3),itr(5)
      ...
      elseif(iz .eq. 1)then
       do ilp=1,5 !ilp
        do ik=1,nhf !ik
         ...
         elseif (distktxy .lt. 1E-5 .and. distk .gt. 1E-5)then
          ikk(ilp)=i_half_klist(ik) !TRS + G1 + G2
          npl(ilp)=nplist(i_half_klist(ik))
          itr(ilp)=1
          isgg(1,ilp)=1
          isgg(2,ilp)=1

         elseif (distkx .lt. 1E-5 .and. distk .gt. 1E-5)then
          ikk(ilp)=i_half_klist(ik) ! + G1
          npl(ilp)=nplist(i_half_klist(ik))
          isgg(1,ilp)=1
         endif
...
!!$*  subroutine for reading basic information
     ...
     &      (xrecilat(i,ik),i=1,3)
      ...
      write(6,*)" "
      write(6,*)"* defaults: -f WAVECAR -kx 2 -ky 2 -s 2 -ii 1 -if VBM 
     &-kp 1"
      ...
      write(6,*)" ex-noMPI)ifort -fpp -assume byterecl -o berry berry.f"
      write(6,*)" ex-MPI)mpif90 -fpp -assume byterecl -o berry berry.f"
