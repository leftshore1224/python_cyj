...
def thermal_flux(
    ...
    - two_term (bool): Use two term expression.
    ...
    if not two_term:
        # @ as a whole
        e_mom = np.sum(positions * np.expand_dims(atomic_energies, axis=2), axis=1)
        J = (e_mom[1:] - e_mom[:-1]) /dt
...
def argparse():
    ...
    parser.add_argument('-t', '--two_term', action='store_true', help="Use two term expression for therm. flux. [default: one term expression]")
    ...
    parser.add_argument('-c', '--w_convec', action='store_true', help="Add up convection term. [default: Don't add up.]")
    parser.add_argument('-e', '--center', action='store_true', help="Center of box is set to be origin of position vectors. [default: Absolute coordinate.]")
...
if __name__ == '__main__':
    ...
    two_term      = args.two_term
    sub_bind      = args.sub_bind
    w_convec      = args.w_convec
    center        = args.center
    ...
    fname = 'kappa-whole/{}_dt{}_c{}_n{}_a{}_t{}_b{}_c{}_e{}.npy'.format(traj_file, args.dt, corr_len, img_slice, avg_intvl, two_term, sub_bind, w_convec, center)
    if load_bool:
        try:
            hfacf = np.load(fname)
    ...
    if not load_bool:
        ...
        if center:
            offset = np.expand_dims(np.sum(latt, axis=1)/2., axis=1)
            posi -= offset
        ...
        if save_bool:
            ...
            call('mkdir kappa-whole/', shell=True)
            np.save(fname, hfacf)
